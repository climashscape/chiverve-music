...
import LyricVertical from './index.vue'

/**
 * 卸载时的定时器清理（2026-09-26 审查）。
 *
 * 症状面：切 `desktopLyric.direction`（垂直 ⇄ 水平）会**卸载重挂**歌词组件，而 useLyric 里
 * 三条定时链原先一条都没清——3s 的自动滚动恢复（`timeout`）、600ms 的延迟滚动
 * （`delayScrollTimeout`）、`scrollXRTo` 内部 10ms 一步的滚动动画（`cancelScrollFn`）。
 * 旧的滚动动画还会在已脱离文档的元素上再跑约 300ms。
 *
 * 这里用假定时器 + `scrollXRTo` 的桩把三件事钉住：
 *   1. 滚轮后挂起的 3s 定时器，卸载时被清掉（`vi.getTimerCount()` 归零）；
 *   2. 开了「延迟滚动」时的 600ms 定时器，卸载时被清掉；
 *   3. 挂载后拿到的滚动取消函数，卸载时被调用（10ms 的动画链不再继续）。
 *
 * `scrollXRTo` 是 useLyric 唯一的滚动出口，桩掉它就不必碰真实 DOM 的滚动行为
 * （jsdom 的 scrollLeft 可写但没有真正的滚动动画）。
 */
const { scrollXRTo } = vi.hoisted(() => ({ scrollXRTo: vi.fn() }))
vi.mock('@common/utils/renderer', () => ({ scrollXRTo }))
...
describe('LyricVertical/useLyric 的卸载清理', () => {
  it('卸载清掉滚轮后的 3s 自动滚动定时器（重挂后没有旧定时器在跑）', async() => {
    const log = vi.spyOn(console, 'log').mockImplementation(() => {})
    const wrapper = mount(LyricVertical)
    await nextTick()
  ...
  it('卸载调用滚动动画的取消函数（10ms 一步的动画不再往已脱离文档的元素上写）', async() => {
    const cancel = vi.fn()
    scrollXRTo.mockReturnValue(cancel)
    lyric.lines = [makeLine()]
    const wrapper = mount(LyricVertical)
    await nextTick()
    await nextTick()
    ...
    expect(scrollXRTo).toHaveBeenCalled()
