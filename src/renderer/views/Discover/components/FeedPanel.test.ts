 ...
 *
 * 用户 2026-09-24 定的口径：「猜你喜欢」板块整体撤掉，且**不再渲染任何标题**
 * ——「推荐」这个 tab 名已经说明了内容就是首页推荐，不必再写一遍「首页推荐」。
 *
...
/** 真实 feed 的 9 个楼层：2 个有名的、1 个占位（type=-1，无目标卡）、6 个无名单卡槽。 */
const realShelves = () => [
...
describe('FeedPanel（工单 05：只剩首页 feed，且一个标题都不渲染）', () => {
  ...
  it('票 20 的并组不回归：3 组卡片、连续 6 个无名楼层并成 1 组（孤卡行数 0）', async() => {
    const wrapper = mountPanel()
    await flushPromises()

    const rows = wrapper.findAll('ul').map(ul => ul.findAll('li').length)
    // [为你打造过滤后 2 张, 最近常听 17 张, 6 个无名楼层并成 1 组 6 张]
    expect(rows).toEqual([2, 17, 6])
